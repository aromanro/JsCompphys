(function () {
            ...
            for (let i = 0; i < this.model.Size; ++i)
                for (let j = 0; j < this.model.Size; ++j)
                    if (!this.model.Boundary(i, j)) {
                        const oldVal = this.model.Field(i, j);
                        const newVal = (this.model.Field(i - 1, j) + this.model.Field(i, j - 1) + this.model.Field(i, j + 1) + this.model.Field(i + 1, j)) / 4.0;
                        this.model.field[this.model.Index(i, j)] = newVal;
                        const dif = oldVal - newVal;
                        change += dif * dif;
                    }
    ...
    function DisplayModel(canvas, model) {
        ...
        const displaySize = canvas.width / model.Size;
        const rectSize = Math.ceil(displaySize);
    ...
    let ParticularModel1 = new SquareModel(8);
    let Relaxation1 = new Relaxation(ParticularModel1);
    let canvas = document.getElementById("relaxationCanvas");

    function Tick() {
        DisplayModel(canvas, Relaxation1.model);
        let error = Relaxation1.Relax();
